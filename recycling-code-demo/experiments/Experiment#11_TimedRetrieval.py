...
from transformers import (
    BertTokenizer, BertForSequenceClassification, DataCollatorWithPadding
)
import datasets
...
def run_model(model: transformers.PreTrainedModel,
...
def tokenize(element: Dict[str, Any],
             column_name: str,
...
def training_main(config: Experiment):
    ...
    model.train()
    vanilla_delta = run_model(model=model,
    ...
    model.to(config.device)
    model.train()
    caching_off_delta = run_model(model=model,
                                  data_loader=data_loader,
                                  step='caching_off')
    del model
    print("-----------------------------------------------------")
    ...
    with caching_hook.record(model):
        caching_on_delta = run_model(model=model,
                                     data_loader=data_loader,
                                     step='caching_on')
