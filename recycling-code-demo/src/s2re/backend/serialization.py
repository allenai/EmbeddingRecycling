...
from typing import Mapping, NamedTuple, Sequence, Type, Union
...
class PickleSerialization:
    @classmethod
    def _raw_load(
        cls: Type["PickleSerialization"], buffer: bytes
    ) -> SerializationContainer:
        return pickle.loads(buffer)

    @classmethod
    def _raw_dump(
        cls: Type["PickleSerialization"], container: SerializationContainer
    ) -> bytes:
        return pickle.dumps(container)

    @classmethod
    def _prepare_serialize_array(
        cls: Type["PickleSerialization"], array: BackendValueType
    ) -> SerializationContainer:
    ...
    def _prepare_serialize_dict(
        cls: Type["PickleSerialization"], di: Mapping[str, BackendValueType]
    ) -> SerializationContainer:
    ...
    def _prepare_serialize_sequence(
        cls: Type["PickleSerialization"], seq: Sequence[BackendValueType]
    ) -> SerializationContainer:
    ...
    @classmethod
    def serialize(
        cls: Type["PickleSerialization"], value: HookSingleValueType
    ) -> bytes:
