...
from ..types import HookComboKeyType, HookComboValueType, BackendValueType
from .serialization import PickleSerialization


class DBProtocol(Protocol):
    def __init__(path: Union[str, Path, None], *args, **kwargs): ...
    def close(self) -> None: ...
...
class BaseKVStorage(ABC):
    ...
    def __init__(self, path: Union[str, Path] = None, read_only: bool = False):
        ...
        self.files = partial(self.files, path=self.path)

    def write(self, key: str, array: BackendValueType) -> None:
        """Write a single element to the storage"""
        return self.batch_write({key: array})

    def read(self, key: str) -> BackendValueType:
        """Reads a single element from the storage"""
        return self.batch_read((key, ))[0]
    ...
    @abstractclassmethod
    def files(cls: Type['BaseKVStorage'], path: Path) -> Iterable[Path]:
...
class BackendRegistry:
    ...
    def set(
        ...
        if hints.get('db', DBProtocol) == DBProtocol:
            raise TypeError(f'{backend_cls.__name__} must have a `db` '
    ...
    def get(cls: Type['BackendRegistry'], name: str) -> Type[BaseKVStorage]:
        ...
        try:
            ...
            raise TypeError(f"No backend with name `{name}`; "
