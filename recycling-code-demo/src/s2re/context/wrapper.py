...
from typing import (Callable, Iterable, List, Sequence, Tuple, Type, Union,
                    Dict, Any, Optional)
...
class StorageWrapper:
    ...
    def _cast_key(
        cls,
        key_to_cast: Union[torch.Tensor, Sequence[torch.Tensor], Sequence[bytes]]
    ) -> List[bytes]:
        """Turns a tensor containing a key into a list of bytes."""
        is_seq = isinstance(key_to_cast, abc.Sequence)
        is_seq_bytes = (is_seq and
                        all(isinstance(elem, bytes) for elem in key_to_cast))
        ...
        return [cls._cast_value(elem, device='cpu').numpy().tobytes()
                for elem in seq_key]
    ...
    def _cast_value(cls,
                    ...
                    device: Union[str, torch.device],
                    requires_grad: bool = False) -> HookComboValueType:
        """Move tensor(s) to cpu, and remove gradients"""

        if isinstance(value, abc.Mapping):
            casted_value = {
                key: cls._cast_value(element,
            ...
            casted_value = [
                cls._cast_value(element,
                                requires_grad=requires_grad,
                                device=device)
                for element in value
            ]
        elif isinstance(value, torch.Tensor):
            casted_value = cls._move_and_grad(value,
                                              grad=requires_grad,
                                              device=device)
    ...
    def store(self,
              ...
              value: Union[torch.Tensor, Sequence[torch.Tensor]]) -> None:
        ...
        seq_val = self._cast_value(seq_val, device='cpu')
        self.storage.batch_write(keys=seq_key, values=seq_val)

    def fetch(self: 'StorageWrapper',
              key: Union[torch.Tensor,
                         Sequence[torch.Tensor],
                         Sequence[bytes]],
              training: bool = False) -> HookComboValueType:
    ...
    def delete(self: 'StorageWrapper',
               key: Union[torch.Tensor,
                          Sequence[torch.Tensor],
                          Sequence[bytes]]) -> None:
...
class FetchAheadStorageWrapper(StorageWrapper):
    ...
    def _start_fetcher_thread(
        ...
        new_thread_factory(
            ...
            kwargs=dict(to_fetch_queue=self._to_fetch_queue,
    ...
    def fetch(
        ...
        seq_val = [self._cast_value(v, device=self.device)
                   for v in seq_val]
    ...
    def store(self, *_, **__) -> None:
        raise NotImplementedError(
            'Cannot write to storage in prefetch mode.'
        )
