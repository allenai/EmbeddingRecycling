...
class CachingHook:
    ...
    def infer_device(module: torch.nn.Module) -> torch.device:
        ...
        if len(device) == 1:
            return device.pop()
        raise ValueError(f'Module {module} has parameters on multiple'
                         f'({len(device):,}) devices; provide device '
                         'manually.')

    @classmethod
    @contextmanager
    def Record(cls: Type['CachingHook'],
               module: torch.nn.Module,
               backend: str,
               path: Union[str, Path],
        ...
        session = CachingSession(recording=True,
                                 training=False,
    ...
    def _check_spec(
        ...
        merged_session_kwargs = {**self.session_kwargs,
                                 **session_kwargs,
                                 **session_args_dict}
        ...
        filtered_session_kwargs = {
            k: v for k, v in merged_session_kwargs.items()
    ...
    def record(self,
               *session_args: Any,
               **session_kwargs: Any) -> Iterator[CachingSession]:
