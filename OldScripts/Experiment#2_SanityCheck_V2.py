...
for dataset in classification_datasets:
    ...
    print("Beginning evaluation using embeddings")

    print(eval_embeddings.shape)

    eval_dataloader = DataLoader(eval_embeddings, batch_size=assigned_batch_size)
    eval_dataloader_labels = DataLoader(eval_labels, batch_size=assigned_batch_size)
    ...
    for batch, labels in zip(eval_dataloader, eval_dataloader_labels):
        ...
        with torch.no_grad():
            ...
            if count == 0:
                ...
                testing_batch = {'ids': torch.FloatTensor([]).to(device), 
                                 'mask': torch.FloatTensor([]).to(device), 
                                 'inputs_embeds': batch.to(device)}

                testing_outputs = new_model(**testing_batch)

                print("Validation Embeddings logits")
                ...
                new_batch = {'ids': torch.FloatTensor([]).to(device), 'mask': torch.FloatTensor([]).to(device), 'inputs_embeds': batch.to(device)}
