...
class CustomBERTModel(nn.Module):
    def __init__(self, number_of_labels, model_choice, dropout_layer, frozen, 
          ...
          if frozen_layer_count > 0:
            ...
            elif model_choice == 'nreimers/MiniLMv2-L6-H768-distilled-from-RoBERTa-Large' or model_choice == 'roberta-large':
          ...
          if frozen_embeddings == True:
            ...
            #print(self.encoderModel.__dict__)
            if model_choice == 'nreimers/MiniLMv2-L6-H768-distilled-from-RoBERTa-Large' or model_choice == 'roberta-large':
          ...
          for param in self.encoderModel.roberta.parameters():
               param.requires_grad = False
    ...
    def forward(self, ids, mask, labels):
            ...
            finetuned_model_embeddings = finetuned_model.roberta.embeddings(ids)

            ############################################

            extended_attention_mask = self.encoderModel.get_extended_attention_mask(mask, finetuned_model_embeddings.size()[:-1], device)
            last_hidden_state = self.encoderModel.roberta.encoder(finetuned_model_embeddings, extended_attention_mask)['last_hidden_state']
...
classification_datasets = ['bc5cdr', 'JNLPBA'] #['bc5cdr', 'JNLPBA', 'NCBI-disease']
data_set_of_finetune = 'NCBI-disease'

########################################################################################
 
checkpoint_path = "checkpoints/experiment4_ner_128.pt"
...
for chosen_learning_rate in learning_rate_choices:
    ...
    for dataset in classification_datasets:
        ...
        print("Dataset of Finetune Choice: " + data_set_of_finetune)
        ...
        # Load pretrained, finetuned encoder

        finetuned_model = AutoModelForTokenClassification.from_pretrained(model_choice, output_hidden_states=True)

        finetuned_path = "./pretrained_roberta_for_experiment4/ner/new_" + data_set_of_finetune #dataset
        finetuned_model.roberta.encoder.load_state_dict(torch.load(finetuned_path), strict=True)
